# LOAD/MCI/CU Sex-specific Effects (using all samples for fitting)
...
chrs <- list.files("/media/Data/WGBS/LOAD_MCI/MCovRaw/BismarkFormatted/chromosomes")
chrs <- chrs[1:22]
loopByChromosome1 <- function(chr, group1, group2, sexOn, outDir) {
for (i in 1:length(chr)) {
...
parser$add_argument("--odir", default= outDir)
...
if (sexOn == FALSE) {
parser$add_argument("--baseline_covariates", default= "bmi,age_at_visit,type1,type2,type3,type4,type5,PC1,PC2")
}
parser$add_argument("--test_covariate", default= "New_Group")
...
# Make new groups for testing
df$New_Group <- paste0(df$diagnostic_group,"_",df$sex)

# set up group factors of interest
allGroups <- c("CONTROL_Female","CONTROL_Male","MCI_Female","MCI_Male","LOAD_Female","LOAD_Male")
groupsOfInterest <- data.frame(c(group1, group2))
notOfInterest <- data.frame(setdiff(allGroups, groupsOfInterest[,1]))
newOrder <- c(groupsOfInterest[,1],notOfInterest[,1])

# Factor groups accordingly depending on what comparison you are making
df$New_Group <- factor(df$New_Group, levels=newOrder)
...
# LOAD v CONTROL
loopByChromosome1(chrs, "CONTROL_Female", "LOAD_Female", sexOn = FALSE, "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_CONTROL_Females/test-diagnostic-group-coded/")
loopByChromosome1(chrs, "CONTROL_Male", "LOAD_Male", sexOn = FALSE, "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_CONTROL_Males/test-diagnostic-group-coded/")
# MCI v CONTROL
loopByChromosome1(chrs, "CONTROL_Female", "MCI_Female", sexOn = FALSE, "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/MCI_CONTROL_Females/test-diagnostic-group-coded/")
loopByChromosome1(chrs, "CONTROL_Male", "MCI_Male", sexOn = FALSE, "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/MCI_CONTROL_Males/test-diagnostic-group-coded/")
# LOAD v MCI
loopByChromosome1(chrs, "MCI_Female", "LOAD_Female", sexOn = FALSE, "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_MCI_Females/test-diagnostic-group-coded/")
loopByChromosome1(chrs, "MCI_Male", "LOAD_Male", sexOn = FALSE, "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_MCI_Males/test-diagnostic-group-coded/")
...
runThePis <- function(idir, odir, sex) {

# set up out directories
opi <- paste0(odir,"/Pis/")
osum <- paste0(odir,"/Summaries/")

parser <- ArgumentParser()
parser$add_argument("--idir", default= idir, help='Where are the models stored')
parser$add_argument("--odir_pis", default= opi, help='Where are the models stored')
parser$add_argument("--odir_summaries", default= osum, help='Where are the models stored')
...
if (sex == "Female") {
	compute_pi_diffs <- function(X, design.df, pi.df){
	    ...
	    design.df <- design.df[grep("Female",design.df$New_Group),]

 	   load.samples <- rownames(design.df[grep("LOAD",design.df$New_Group),])
 	   mci.samples <- rownames(design.df[grep("MCI",design.df$New_Group),])
  	  control.samples <- rownames(design.df[grep("CONTROL",design.df$New_Group),])
...
compute_pi_diffs <- function(X, design.df, pi.df){
    ...
    design.df <- design.df[grep("Male",design.df$New_Group),]
...
# Females (LOAD v CONTROL)
runThePis("/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_CONTROL_Females/test-diagnostic-group-coded/", "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_CONTROL_Females/Outputs/", "Female")
# Males (LOAD v CONTROL)
runThePis("/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_CONTROL_Males/test-diagnostic-group-coded/", "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_CONTROL_Males/Outputs/", "Male")
# Females (MCI v CONTROL)
runThePis("/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/MCI_CONTROL_Females/test-diagnostic-group-coded/", "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/MCI_CONTROL_Females/Outputs/", "Female")
# Males (MCI v CONTROL)
runThePis("/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/MCI_CONTROL_Males/test-diagnostic-group-coded/", "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/MCI_CONTROL_Males/Outputs/", "Male")
# Females (LOAD v MCI)
runThePis("/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_MCI_Females/test-diagnostic-group-coded/", "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_MCI_Females/Outputs/", "Female")
# Males (LOAD v MCI)
runThePis("/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_MCI_Males/test-diagnostic-group-coded/", "/media/Data/WGBS/LOAD_MCI/Results/Sex_Specific_Effects/LOAD_MCI_Males/Outputs/", "Male")
